...
class TestModifyImage(unittest.TestCase):
    ...
    def test_file_path_doesnt_exist(self):
        result = modify_image('blablabla', np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2]]))
    ...
    def test_file_is_not_jp2_image(self):
        result = modify_image('input/cake.jpg', np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2]]))
        self.assertEqual(result.args[0], 'The file is not a jp2 image')

    @patch('modules.files_paths.exists',Mock(return_value = True))
    @patch('modules.files_paths.splitext',Mock(return_value = ('input/cake','.jp2')))
    @patch('modules.modify_image.full_polygon', Mock(side_effect=ValueError('Invalid polygon frame')))
    def test_polygon_frame_is_invalid(self):
        result =  modify_image('input/cake.jp2', 'blablabla')
        self.assertEqual(result.args[0],'Invalid polygon frame')
    ...
    def test_success_blackening_an_image(self,exists,splitext):
        result = modify_image('input/cake.jp2', np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2]]))
...
class TestFullPolygon(unittest.TestCase):
    ...
    @patch('modules.modify_image.fill_polygon', Mock(side_effect=ValueError('Invalid polygon frame')))                       
    def test_polygon_frame_is_invalid(self):
        with self.assertRaises(Exception) as context:
           full_polygon('blablabla')
        self.assertEqual(str(context.exception), 'Invalid polygon frame')

    @patch('modules.modify_image.fill_polygon', Mock(return_value=np.array([[5,2] ,[6,2] ,[7,3] ,[8,3] ,[7,4]])))
    def test_polygon_frame_is_valid(self):
        result = full_polygon(np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2]] ))
        np.testing.assert_almost_equal(result, np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2], [5,2] ,[6,2] ,[7,3] ,[8,3] ,[7,4]] ))
...
class TestFillPolygon(unittest.TestCase):
    ...
    def test_polygon_frame_is_valid(self):
        result = fill_polygon(np.array([[5,1] ,[6,1], [7,2], [8,2] ,[9,1], [9,2] ,[9,3], [8,4], [7,5], [6,4], [6,3], [5,3] ,[4,2]] ))
        np.testing.assert_almost_equal(result, np.array([[5,2] ,[6,2] ,[7,3] ,[7,4] ,[8,3]]))

class TestOpenReadWriteImage(unittest.TestCase):

    @patch('modules.modify_image.rasterio.open')
    def test_the_matrix_is_not_valid(self,open):
        with self.assertRaises(Exception) as context:
           open_read_write_image('input/cake.jp2', 'blablabla')
        self.assertEqual(str(context.exception), 'The matrix is not valid')
