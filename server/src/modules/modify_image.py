...
from .files_paths import path_exist, image_in_required_format
...
def fill_polygon(polygon_frame):
    ...
    return np.array(
        [
            [x, y]
            for x in range(int(minx) + 1, int(maxx))
            for y in range(int(miny) + 1, int(maxy))
            if Polygon(polygon_frame).contains(Point(x, y))
        ]
    )
